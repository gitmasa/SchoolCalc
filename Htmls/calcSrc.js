...
(function($, undefined){
	...
	calcSrc.getSubtractionWithCarryDown_3digit_2digit = function(){
		var first = calcSrc.getIntPair(1);
		var second = calcSrc.getIntPair(1);
		var tmp = 0;
		if (second[0] > second[1]) {
		...
		return [calcSrc.getRandom(1,9)+''+second[0]+''+first[0], '-', second[1]+''+first[1]];
	};
	...
	calcSrc.getMultiple_2digit_2digit = function(){
		var a1 = calcSrc.getRandom(10,99)+'';
		var a2 = calcSrc.getRandom(10,99)+'';
		return [a1, '×', a2];
	};
	...
	calcSrc.classes.ResultDb = (function(){
		...
		_skelton.prototype = {
			...
			formatTime: function(dtStr){
				...
				var strMin = ''+tmp.getMinutes();
				var strHour = ''+tmp.getHours();
				if (strMin.length === 1) {
					strMin = '0'+strMin;
				}
				if (strHour.length === 1) {
				...
				return tmp.getFullYear()+'年'+(tmp.getMonth()+1)+'月'+tmp.getDate()+'日 '+strHour+':'+strMin;
			}
		};
		return _skelton;
	})();
	...
	calcSrc.classes.QuestionBuilder = (function(){
		var _skelton = function(level){
			var me = this;
			me.level = parseInt(level,10);
			me.level = me.level ? me.level : 1;
			me.multiFnc = function(){
				var a1 = me.level < 2 ? calcSrc.getRandom(2, 9) : calcSrc.getRandom(11, 99);
				var a2 = me.level < 3 ? calcSrc.getRandom(2, 9) : calcSrc.getRandom(11, 99);
				return [''+a1, '×', ''+a2];
			};
			me.addFnc = function(){
				return me.level < 2 ? calcSrc.getAdditionWithCarryUp(2) : calcSrc.getAdditionWithCarryUp(3);
			};
			me.subFnc = function(){
				return me.level < 2
					? calcSrc.getSubtractionWithCarryDown_2digit_2digit()
					: calcSrc.getSubtractionWithCarryDown_3digit_2digit();
			};
			me.divFnc = function(){
				return me.level < 2
					? calcSrc.getDivision_2digit_1digit()
					: calcSrc.getDivision_3digit_1digit();
			};
		};
		_skelton.prototype = {
			_aSet: [
	...
	calcSrc.classes.Calculator = (function(){
		...
		_skelton.prototype = {
			getQuestion: function(order){
				...
				else if (ope === '*rev') { // 分数 => 小数
					var rnd = calcSrc.getRandom(0,999) % 2;
					var item1 = $('<span>')
							.attr({'data-value': order.formula[0]})
							.addClass(me.config.firstOperandClass).text(order.formula[0]);
					var item2 = $('<input type="'+inputType+'">').attr('size', inputSize).addClass(me.config.firstAnswerClass);
			...
			check: function($parentDom){
				...
				$lines.each(function (a, lineRaw) {
					...
					if (ope === '+' || ope === '-' || ope === '×'
						|| ope === '÷' || ope === '*rev' || ope === '%' || ope === 'gcd' || ope === 'lcm') {
						...
						second = parseFloat(second);
						ans1 = parseFloat(ans1);
						valid = Math.abs((first / second) - ans1) < 0.0000001;
					}
	...
	calcSrc.classes.JpRomans = (function(){
		var skelton = function(){
		...
		skelton.prototype = {
			romansDic: {
				...
				'た':['ta'], 'ち':['ti', 'thi', 'chi'], 'つ':['tu', 'tsu'], 'て':['te'], 'と':['to'],
				...
				'ぢゃ':['zya', 'jya', 'ja'], 'ぢゅ':['zyu', 'jyu', 'ju'], 'ぢょ':['zyo', 'jyo', 'jo'],
			...
			tsuExcludes:{'あ':1,'い':1,'う':1,'え':1,'お':1,'ん':1,'っ':1},
			kanas: [],
			_makeKanas: function(){
				var me = this;
				if (me.kanas.length > 0) {
					return;
				}
				$.each(me.romansDic, function(key,val){
					me.kanas.push(key);
				});
			},
			buildWord: function(len){
				var me = this;
				me._makeKanas();
				len = parseInt(len, 10);
				if (len < 0 || !len) {
				...
				var kanaLen = me.kanas.length;
				var ret = '';
				for (var i=0; i<len; i++) {
					var rnd = calcSrc.getRandom(0, kanaLen);
					if (rnd === kanaLen) {
						//[っ]
						var preChr = ret.substr(-1);
						if (me.tsuExcludes[preChr]) {
							rnd = calcSrc.getRandom(0, kanaLen-1);
							ret += me.kanas[rnd];
						} else {
							ret += 'っ';
						}
			...
			buildAnswer: function(kanaStr) {
				var wordLen = kanaStr.length;
				var rets = [];
				var i = 0;
				var preChr = '';
				var offset = 0;
				while(i < wordLen) {
					var chr = kanaStr.substr(i, 1);
					if (chr === 'っ') {
						preChr = chr;
					}
					if (chr === 'ゃ' || chr === 'ゅ' || chr === 'ょ') {
